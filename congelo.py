...
def load_stock():
    ...
    # SÉCURITÉ : Conversion des types pour éviter les erreurs de calcul
    df_raw['nombre'] = pd.to_numeric(df_raw['nombre'], errors='coerce').fillna(0).astype(int)
    # On crée 'date_dt' pour les calculs internes, 'errors=coerce' transforme les dates invalides en NaT (Not a Time)
...
with tab1:
    ...
    if working_df.empty:
        ...
        for _, row in working_df.iterrows():
            ...
            # SÉCURITÉ : Calcul de date robuste
...
with tab_recap:
    ...
    if not liste_lieux_form:
        ...
        lieu_recap = st.radio("Lieu :", liste_lieux_form, horizontal=True, key="radio_recap")
        recap_df = df.copy()
